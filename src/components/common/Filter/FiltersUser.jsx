...
const FiltersUser = ({ closeModal }) => {
  const { filter, isLoading, users } = useSelector((state) => ({
    filter: state.filter,
  ...
  const max = selectMax(users);
  const min = selectMin(users);
  ...
  return (
    <div className="filters">
      ...
      <div className="filters-field">
        ...
        <br />
        <br />
        {(users.length === 0 && isLoading) || max === 0 ? (
          <h5 className="text-subtle">Loading Filter</h5>
        ) : users.length === 1 ? (
...
FiltersUser.propTypes = {
  closeModal: PropType.func.isRequired
};

export default withRouter(FiltersUser);
