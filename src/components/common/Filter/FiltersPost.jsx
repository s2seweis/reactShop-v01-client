...
import { selectMax, selectMin } from 'selectors/selector_post';
import PriceRange from '../PriceRange';

const FiltersPost = ({ closeModal }) => {
  const { filter, isLoading, posts } = useSelector((state) => ({
    filter: state.filter,
  ...
  const max = selectMax(posts);
  const min = selectMin(posts);
  ...
  return (
    <div className="filters">
      ...
      <div className="filters-field">
        ...
        <br />
        <br />
        {(posts.length === 0 && isLoading) || max === 0 ? (
          <h5 className="text-subtle">Loading Filter</h5>
        ) : posts.length === 1 ? (
...
FiltersPost.propTypes = {
  closeModal: PropType.func.isRequired
};

export default withRouter(FiltersPost);
